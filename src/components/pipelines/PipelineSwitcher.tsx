...
interface PipelineRowProps {
  ...
  getPipelineColor: (p: Pipeline) => string;
  getStatusText: (p: Pipeline) => string;
  t: (key: string, opts?: Record<string, unknown>) => string;
}

function PipelineRow({ pipeline, selected, onSelect, getPipelineColor, getStatusText, t }: PipelineRowProps) {
  return (
    <DropdownMenuItem
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {pipeline.description && (
          <div className="text-xs text-muted-foreground truncate mb-1">{pipeline.description}</div>
        )}
      ...
      {selected && <div className="w-2 h-2 rounded-full bg-primary flex-shrink-0 mt-3" />}
    </DropdownMenuItem>
  );
}
...
export default function PipelineSwitcher({
  ...
  const groupedPipelines = useMemo(() => {
    const sale = filteredPipelines.filter(p => p.pipeline_type === 'sale');
    const rental = filteredPipelines.filter(p => p.pipeline_type === 'rental');
    const others = filteredPipelines.filter(p => !REAL_ESTATE_TYPES.includes(p.pipeline_type));
    return { sale, rental, others };
  }, [filteredPipelines]);

  const hasGroups = groupedPipelines.sale.length > 0 || groupedPipelines.rental.length > 0;
        ...
        <div className="flex flex-col">
          ...
          <div className="max-h-[300px] overflow-y-auto p-1 bg-background">
            {filteredPipelines.length === 0 ? (
              ...
              <>
                {groupedPipelines.sale.length > 0 && (
                  <>
                    <div className="px-3 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                      Venda
                    </div>
                    {groupedPipelines.sale.map(pipeline => (
                      <PipelineRow
                        key={pipeline.id}
                        pipeline={pipeline}
                        selected={selectedPipeline?.id === pipeline.id}
                        onSelect={handlePipelineSelect}
                        getPipelineColor={getPipelineColor}
                ...
                {groupedPipelines.others.length > 0 && (
                  <>
                    <div className="px-3 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider mt-1">
                      Outros
                    </div>
                    {groupedPipelines.others.map(pipeline => (
                      <PipelineRow
                        key={pipeline.id}
                        pipeline={pipeline}
                        selected={selectedPipeline?.id === pipeline.id}
